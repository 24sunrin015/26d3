...
![y25task_img1.png](./images/y25task_img1.png)
...
| --- | --- |
| AWS | 개발언어/프레임워크 |
| - VPC  - ECS, EC2, ECR  - RDS, DynamoDB | - Golang / Gin |
...
| --- | --- |
| AWS | 개발언어/프레임워크 |
| - ELB, API Gateway  - CloudFront, R53  - S3  - Cloudwatch  - WAF | - Docker |
...
user 어플리케이션은 관계형 데이터베이스를 사용하며 읽기, 쓰기 작업이 모두 가능해야 합니다. 어플리케이션은 MySQL Community 엔진을 지원합니다. DB 인스턴스는 최소한으로 운영합니다.
...
|  |
| --- |
| CREATE TABLE user (  id VARCHAR(255) NOT NULL,  username VARCHAR(255) NOT NULL,  email VARCHAR(255) NOT NULL,  status\_message VARCHAR(255) NOT NULL,  PRIMARY KEY (userid),  UNIQUE KEY uk\_username (username)  ); |
...
product 어플리케이션은 DynamoDB를 사용하며 읽기, 쓰기 작업이 모두 가능해야합니다. 테이블은 1개만 생성하도록 하며, 테이블에 저장하는 데이터의 예제 포맷은 다음과 같습니다. 어플리케이션 동작과 트래픽 패턴에 맞게 PK, SK, Index의 설계가 필요할 수 있습니다.

|  |  |  |
| --- | --- | --- |
| Key | Data Type | Description |
| id | string | product 고유값 |
| name | string | product 이름 |
| price | Number | product 가격 |
...
| /v1/user | POST | {  "requestid":"999999999999",  "uuid":"7c5a3c6a-758f-4bc5-9bdf-3e573a0ad729",  "username":"dbdump500001",  "email":"dbdump500001@example.org",  "status\_message":"I’m happy",  } | 201 |
...
만약 /v1/user POST 요청 시 email 주소의 포맷이  가 아닌 경우 403 에러가 발생하도록 구성하세요. 이는 어플리케이션 로직으로 구현되어 있지 않습니다.
...
제품 정보를 관리하는 API를 제공합니다. 어플리케이션의 동작은 아래를 참고합니다. product API는 제품 생성(POST 요청) 이후 해당 제품에 대한 정보는 변경되지 않으며 동일한 id에 대한 요청이 빈번하게 발생될 것으로 예상됩니다. product API는 최소한의 가용성을 위해 5초 이하의 응답시간을 보장하고자 하며, 0.2초 이하의 응답시간을 서비스 수준 목표로 설정하였습니다. 해당 기준으로 서비스 수준을 측정하고 어플리케이션을 운영하도록 합니다.
...
| --- | --- | --- | --- |
| Environment Key | Data Type | Value | etc |
| TABLE\_NAME | string | DynamoDB 테이블 이름 | required |
| TABLE\_INDEX\_NAME | string | DynamoDB 테이블 내 인덱스 이름  (default: 인덱스를 사용하지 않음) | optional |
...
어플리케이션을 배포하고 외부 사용자들에게 서비스를 제공할 수 있도록 시스템을 구축합니다. 컨테이너를 활용해야 하며 컨테이너 오케스트레이션 툴로는 ECS를 사용합니다. EKS를 사용할 수 없습니다. 컨테이너 이미지 레포지토리로는 ECR을 사용할 수 있으며 컨테이너 호스팅을 위한 컴퓨팅 리소스는 EC2를 사용합니다.
